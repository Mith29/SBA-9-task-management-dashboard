...
function TaskItem({ task, onStatusChange, onDelete, onEdit }: TaskItemProps) {
  ...
  return (
    <div className="task-card p-4 mb-2 w-full bg-white rounded-[12px] border border-gray-300 shadow-sm hover:shadow-md transition-shadow">
      {/* Header */}
      <div className="task-header flex justify-between items-start">
        ...
        <div className="task-actions flex items-center gap-2">
          ...
          <button
            onClick={handleEdit}
            className="edit-btn  cursor-pointer hover:text-blue-700 text-blue-500"
          >
          <PencilIcon className="h-7 w-5"/>
          </button>

          <button
            onClick={handleDelete}
            className="delete-btn  cursor-pointer hover:text-red-700 text-red-500"
          >
